...
export default function EnglishTeaserPage() {
  ...
  const submitGuideForm = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/register', {
        ...
        body: JSON.stringify({ type: '英ガイド登録', name: gName, email: gEmail, category: gCat }),
      });
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .hero-sub { font-size: 16px; color: var(--text2); max-width: 500px; margin: 0 auto 10px; line-height: 1.8; }
        ...
        .counter-label { font-size: 11px; color: var(--text3); margin-top: 3px; }
        ...
        .section-desc { font-size: 14px; color: var(--text2); text-align: center; margin-bottom: 48px; }
        ...
        .reg-card .card-desc { font-size: 12.5px; color: var(--text2); line-height: 1.65; margin-bottom: 20px; }
        .reg-perks { list-style: none; margin-bottom: 24px; padding: 0; }
        .reg-perks li { font-size: 12px; color: var(--text2); padding: 4px 0; display: flex; align-items: flex-start; gap: 8px; }
        ...
        .agree-label { display: flex; align-items: flex-start; gap: 8px; font-size: 11.5px; color: var(--text2); margin: 6px 0 12px; cursor: pointer; line-height: 1.5; }
        ...
        .success-body { font-size: 12.5px; color: var(--text2); line-height: 1.65; }
        ...
        .concept-card p { font-size: 12px; color: var(--text2); line-height: 1.65; }
        ...
        .tl-sub { font-size: 10.5px; color: var(--text3); line-height: 1.4; }
        ...
        .cta-section p { font-size: 14px; color: rgba(255,255,255,0.55); margin-bottom: 36px; line-height: 1.8; }
      ...
      <div className="banner">🎉 cocora is currently preparing for launch. Pre-register now to get <span>early access rewards</span>!</div>
      ...
      <nav>
        ...
        <div className="nav-right">
          <div className="launch-pill"><div className="launch-dot"></div>Launching in 2026</div>
          <button className="nav-btn" onClick={() => scrollToReg()}>Pre-register</button>
        </div>
      </nav>

      <section className="hero">
        <div className="hero-tag">🇯🇵 Niche Experiences, Real Connections</div>
        <h1>Your "Passions"<br /><span className="accent">Will Transform Travel.</span></h1>
        <p className="hero-sub">Connects local Japanese guides with global travelers. Experience road cycling, anime pilgrimages, and local hidden food tours with a guide who shares your exact hobbies.</p>
        <p className="hero-launch">Official launch is scheduled for <strong>2026</strong>. Register now to claim your spot on the waitlist.</p>
        <div className="hero-ctas">
          <button className="btn-dark" onClick={() => scrollToReg('guide')}>Pre-register as a Guide →</button>
          <button className="btn-outline" onClick={() => scrollToReg('traveler')}>Pre-register as a Traveler</button>
        </div>
      </section>
      ...
      <div className="counter-bar">
        <div className="counter">
          ...
          <div className="counter-label">Guides Registered</div>
        </div>
        <div className="counter">
          <div className="counter-num blue">{travelerCount}</div>
          <div className="counter-label">Travelers on Waitlist</div>
        </div>
        <div className="counter">
          <div className="counter-num">89</div>
          <div className="counter-label">Planned Categories</div>
        </div>
        <div className="counter">
          <div className="counter-num">47 Prefectures</div>
          <div className="counter-label">Covered Areas</div>
        </div>
      </div>

      <section className="reg-section" ref={regSectionRef} id="registration">
        <div className="section-label">EARLY ACCESS</div>
        <div className="section-title">Join the Waitlist & Get Rewards</div>
        <div className="section-desc">Exclusive benefits are available for both guides and travelers. Secure your early access today.</div>

        <div className="reg-grid">
          {/* GUIDE CARD */}
          <div className="reg-card guide">
            <div className="reg-badge guide">🗾 Become a Local Guide</div>
            <h2>Turn Hobbies Into Income.<br />Guide Travelers as a Side Hustle.</h2>
            <p className="card-desc">Share what you love most with global travelers. No professional license or special certificate required.</p>
            <ul className="reg-perks">
              <li><div className="perk-check coral">✓</div>Highest priority notification upon launch</li>
              <li><div className="perk-check coral">✓</div><strong>0%</strong> platform fee for your first month</li>
              <li><div className="perk-check coral">✓</div>Early-bird tier upgrades for initial users</li>
              <li><div className="perk-check coral">✓</div>Free profile creation and optimization support</li>
            </ul>
            
            {!isGuideSubmitted ? (
              <form className="reg-form" onSubmit={submitGuideForm}>
                <input type="text" placeholder="Your Name" required ref={guideNameRef} value={gName} onChange={(e) => setGName(e.target.value)} className="g" />
                <input type="email" placeholder="Email Address" required value={gEmail} onChange={(e) => setGEmail(e.target.value)} className="g" />
                <select value={gCat} onChange={(e) => setGCat(e.target.value)}>
                  <option value="" disabled>Select your primary expertise (Optional)</option>
                  <option>🚴 Road Cycling &amp; Bike Tours</option>
                  <option>⛩️ Anime Culture &amp; Pilgrimages</option>
                  <option>🍜 Local Hidden Food Tours</option>
                  <option>📷 Street Photography Walks</option>
                  <option>🏔️ Hiking &amp; Trekking</option>
                  <option>🎮 Gaming &amp; Subculture Exchange</option>
                  <option>🎋 Traditional Tea Ceremony / Martial Arts</option>
                  <option>🧘 Zen &amp; Mindfulness Experiences</option>
                ...
                <label className="agree-label g">
                  ...
                  <span>I agree to the <Link href="/en/terms" target="_blank">Terms of Service</Link> and <Link href="/en/privacy" target="_blank">Privacy Policy</Link>.</span>
                </label>
                
                <button type="submit" disabled={isSubmittingGuide} className="submit-btn guide">
                  {isSubmittingGuide ? 'Sending...' : 'Register as Local Guide →'}
                </button>
              </form>
            ) : (
              <div className="success-msg" style={{ display: 'block' }}>
                <div className="success-icon">🎉</div>
                <div className="success-title">Thank you for registering!</div>
                <div className="success-body">We will contact you first as soon as our service launches.<br />Check your inbox for further reward details.</div>
              </div>
            )}
          </div>
          ...
          <div className="reg-card traveler">
            <div className="reg-badge traveler">✈️ Register as a Traveler</div>
            <h2>Dive Into Niche Trips.<br />Discover the Real Japan.</h2>
            <p className="card-desc">Uncover experiences you won't find in any standard guidebook, spending meaningful time with local Japanese peers.</p>
            <ul className="reg-perks">
              <li><div className="perk-check blue">✓</div>Highest priority notice when platform goes live</li>
              <li><div className="perk-check blue">✓</div>Instant <strong className="text-blue">10% OFF</strong> coupon for your first booking</li>
              <li><div className="perk-check blue">✓</div>Priority access to pre-registered elite guides</li>
              <li><div className="perk-check blue">✓</div>VIP English customer support priority lines</li>
            </ul>
            ...
            {!isTravelerSubmitted ? (
              <form className="reg-form" onSubmit={submitTravelerForm}>
                ...
                <label className="agree-label t">
                  ...
                  <span>I agree to the <Link href="/en/terms" target="_blank">Terms</Link> and <Link href="/en/privacy" target="_blank">Privacy Policy</Link>.</span>
                </label>
      ...
      <section className="section">
        ...
        <div className="tabs">
          <button className={`tab ${activeTab === 'tourist' ? 'active' : ''}`} onClick={() => switchTab('tourist')}>For Travelers</button>
          <button className={`tab ${activeTab === 'guide' ? 'active' : ''}`} onClick={() => switchTab('guide')}>For Local Guides</button>
        </div>
        ...
        {activeTab === 'tourist' && (
          <div className="step-content active steps">
            <div className="step">
              ...
              <h3>Submit Your Trip</h3>
              <p>Enter your travel dates, target activities, and style preferences. Feel free to list highly niche requests.</p>
            </div>
            <div className="step">
              <div className="step-num">02</div>
              <div className="step-icon blue"><i className="fa fa-comments"></i></div>
              <h3>Receive Offers</h3>
              <p>Local guides sharing your passions will send custom tour offers. Pick your favorite by browsing profiles.</p>
            </div>
            <div className="step">
              <div className="step-num">03</div>
              <div className="step-icon purple"><i className="fa fa-lock"></i></div>
              <h3>Secure Booking</h3>
              <p>Pay safely with credit card encryption. Share your experience by leaving a review afterwards.</p>
            </div>
            <div className="step">
              <div className="step-num">04</div>
              <div className="step-icon coral"><i className="fa fa-star"></i></div>
              <h3>Enjoy Real Japan</h3>
              <p>Authentic interaction with locals. Form natural friendships, just like exploring with a resident buddy.</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'guide' && (
          <div className="step-content active steps">
            <div className="step">
              ...
              <p>To ensure platform safety and community trust, all guides verify their identity using official state IDs.</p>
            </div>
            <div className="step">
              <div className="step-num">02</div>
              <div className="step-icon blue"><i className="fa fa-user-pen"></i></div>
              <h3>Build Your Profile</h3>
              <p>List your unique hobbies and favorite local spots. Whether it's cycling routes or arcade gaming, anything goes.</p>
            </div>
            <div className="step">
              <div className="step-num">03</div>
              <div className="step-icon purple"><i className="fa fa-yen-sign"></i></div>
              <h3>Set Your Hourly Rate</h3>
              <p>You are in complete control. Set your custom hourly fees. The more reviews you gain, the more you can earn.</p>
            </div>
            <div className="step">
              <div className="step-num">04</div>
              <div className="step-icon coral"><i className="fa fa-handshake"></i></div>
              <h3>Start Guiding</h3>
              <p>Proactively send tour offers to incoming global travelers or accept booking inquiries to begin your journey.</p>
            </div>
          </div>
        )}
      </section>
      ...
      <section className="concept-section">
        ...
        <div className="section-title">What is cocora?</div>
        <div className="section-desc" style={{ marginBottom: '40px' }}>Ditching traditional sightseeing for an entirely new form of deep travel accompaniment.</div>
        <div className="concept-grid">
          <div className="concept-card">
            <span className="concept-emoji">🤝</span>
            <h3>Explore Like Friends</h3>
            <p>They aren't textbook tour operators—they are residents who love what you love. Travel freely, just like roaming around with a local friend.</p>
          </div>
          <div className="concept-card">
            <span className="concept-emoji">🎯</span>
            <h3>Hyper-Focused on Niche</h3>
            <p>Road cycling runs, anime sacred spot hunting, midnight food alley culture—we only provide deep trips missing from guidebooks.</p>
          </div>
          <div className="concept-card">
            <span className="concept-emoji">🔒</span>
            <h3>Safe & Secure by Design</h3>
            <p>Every single guide goes through strict ID verification. Payments are held securely in escrow with dedicated customer support.</p>
          </div>
        </div>
      </section>
      ...
      <section className="cats-section">
        ...
        <div className="section-title">Incredible Journeys Await You</div>
        <div className="section-desc" style={{ marginBottom: '40px' }}>Initial theme categories available immediately at launch.</div>
        <div className="cats-grid">
          <div className="cat"><span className="cat-emoji">🚴</span><div className="cat-name">Road Cycling</div><div className="cat-soon">Coming Soon</div></div>
          <div className="cat"><span className="cat-emoji">⛩️</span><div className="cat-name">Anime Pilgrimages</div><div className="cat-soon">Coming Soon</div></div>
          <div className="cat"><span className="cat-emoji">🍜</span><div className="cat-name">Hidden Local Food</div><div className="cat-soon">Coming Soon</div></div>
          <div className="cat"><span className="cat-emoji">📷</span><div className="cat-name">Street Photo Walks</div><div className="cat-soon">Coming Soon</div></div>
          <div className="cat"><span className="cat-emoji">🏔️</span><div className="cat-name">Mountain Hiking</div><div className="cat-soon">Coming Soon</div></div>
          <div className="cat"><span className="cat-emoji">🎮</span><div className="cat-name">Gaming &amp; Subculture</div><div className="cat-soon">Coming Soon</div></div>
          <div className="cat"><span className="cat-emoji">🎋</span><div className="cat-name">Tea &amp; Martial Arts</div><div className="cat-soon">Coming Soon</div></div>
          <div className="cat"><span className="cat-emoji">🧘</span><div className="cat-name">Zen &amp; Meditation</div><div className="cat-soon">Coming Soon</div></div>
        </div>
      </section>
      
      <section className="guides-section">
        <div className="section-label">FEATURED GUIDES</div>
        <h2>Highly Rated Local Guides</h2>
        <div className="guides-grid">
          <div className="guide-card">
            <div className="guide-img g1">🚴<div className="rank-badge">GOLD</div></div>
            <div className="guide-info">
              <div className="guide-name">Kenji T. · Tokyo</div>
              <div className="guide-tags">
                <span className="guide-tag coral">Road Cycling</span>
                <span className="guide-tag">Arakawa Trails</span>
                <span className="guide-tag">Mt. Fuji Base</span>
              </div>
              <div className="guide-meta">
                <div className="guide-price">¥3,500<span>/hr</span></div>
          ...
          <div className="guide-card">
            ...
            <div className="guide-info">
              <div className="guide-name">Yuki M. · Kyoto/Osaka</div>
              <div className="guide-tags">
                <span className="guide-tag coral">Pilgrimage</span>
                <span className="guide-tag">Anime Otaku</span>
                <span className="guide-tag">Cosplay</span>
              </div>
              <div className="guide-meta">
                <div className="guide-price">¥5,000<span>/hr</span></div>
          ...
          <div className="guide-card">
            ...
            <div className="guide-info">
              <div className="guide-name">Hana S. · Fukuoka</div>
              <div className="guide-tags">
                <span className="guide-tag coral">Local Food</span>
                <span className="guide-tag">Yatai Stalls</span>
                <span className="guide-tag">Izakaya Hop</span>
              </div>
              <div className="guide-meta">
                <div className="guide-price">¥2,200<span>/hr</span></div>
      ...
      <section className="timeline-section">
        ...
        <h2>Launch Schedule & Milestones</h2>
        <div className="section-desc" style={{ marginBottom: '44px' }}>We are currently in the early-access waitlist phase.</div>
        <div className="timeline">
          <div className="tl-step">
            <div className="tl-dot done"></div>
            <div className="tl-label done">✓ Done</div>
            <div className="tl-sub">Core Concept<br />Requirements Defined</div>
          </div>
          <div className="tl-step">
            <div className="tl-dot active"></div>
            <div className="tl-label active">← Current</div>
            <div className="tl-sub">Pre-registration<br />Now Open</div>
          </div>
          <div className="tl-step">
            <div className="tl-dot"></div>
            <div className="tl-label">Development</div>
            <div className="tl-sub">Platform Core Architecture</div>
          </div>
          <div className="tl-step">
            <div className="tl-dot"></div>
            <div className="tl-label">β Version</div>
            <div className="tl-sub">Closed Beta Test for Waitlist Users</div>
          </div>
          <div className="tl-step">
            <div className="tl-dot"></div>
            <div className="tl-label">Official Launch</div>
            <div className="tl-sub">Full Service Rollout in 2026</div>
          </div>
          <div className="tl-step">
            <div className="tl-dot"></div>
            <div className="tl-label">Expansion</div>
            <div className="tl-sub">Japan Nationwide & Mobile App</div>
          </div>
        </div>
      </section>

      <section className="cta-section">
        <h2>Bring Your Hobbies, <br /><span className="accent">Completely Change Someone's Journey.</span></h2>
        <p>Perfect as a side hustle or extension of your hobbies. No special qualifications needed.<br />Your local insights and passion will become unforgettable treasures for global travelers.</p>
        <div className="cta-btns">
          <button className="btn-white" onClick={() => scrollToReg('guide')}>Join as Local Guide</button>
          <button className="btn-ghost" onClick={() => scrollToReg('traveler')}>Join the Traveler Waitlist</button>
        </div>
      </section>
      ...
      <footer>
        <div className="footer-inner">
          ...
          <div className="footer-links">
            ...
            <a href="https://mefar.jp/contact" target="_blank" rel="noopener noreferrer">Contact Us</a>
          </div>
        </div>
      </footer>
    </>
